...
def createUser(user_email, customerId,tenant_user,tenant_password,dashboard_id):
    ...
    with RestClientCE(base_url=url) as rest_client:
        try:
            ...
            r=requests.post(url=url+"/api/user?sendActivationMail=true",headers={"X-Authorization":"Bearer "+rest_client.token_info.get("token")}, json= config)
            return r.json()['id']['id'],r.status_code
        except:
            return False,500